...
from actions import ACTION_MESSAGE, ROTATE_CLOCKWISE, \
    ROTATE_COUNTER_CLOCKWISE, SWAP_HORIZONTAL, SWAP_VERTICAL, SMASH, PASS, \
    PAINT, COMBINE, ACTION_PENALTY
...
def _block_to_squares(board: Block) -> List[Tuple[Tuple[int, int, int],
    ...
    """
    # if board colour is none, means no more children to check
    if board.colour is not None:
        return [(board.colour, board.position, board.size)]
    else:
        lst = []
        for item in board.children:
            # temp is [(colour, position, size)]
            temp = _block_to_squares(item)
            # just need to extend to list, as order does not matter
            lst.extend(temp)
        return lst
...
class MainState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Processes the event for the player"""
        self._current_player().process_event(event)

    def update(self) -> GameState:
        """Updates the game state after player places a move"""
    ...
    def render(self, renderer: Renderer) -> None:
        """Renders the board after a move is placed"""
...
class AnimateMoveState(GameState):
   ...
   """
    ...
    def render(self, renderer: Renderer) -> None:
        """Render the board after a move is placed"""
...
class GameOverState(GameState):
    ...
    def update(self) -> GameState:
        """Update the board after a move placed"""
        # Nothing to change
        return self
